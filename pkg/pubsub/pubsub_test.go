...
func TestPubSub(t *testing.T) {
	...
	t.Run("Create subscription", func(t *testing.T) {
		...
		subscription, err := client.CreateSubscription(ctx, &pubsub.SubscriptionConfig{
	...
	t.Run("Create subscription fails because exists", func(t *testing.T) {
		subName := "subscription"
		_, err := client.CreateSubscription(ctx, &pubsub.SubscriptionConfig{
			Project: cfg.ProjectID,
			Topic:   topicName,
			Name:    subName,
		})
	...
	t.Run("Get or create topic with existing subscription", func(t *testing.T) {
		subName := "subscription"
		fullyQualifiedName := fmt.Sprintf("projects/%v/subscriptions/%v", cfg.ProjectID, subName)
		subscription, err := client.GetOrCreateSubscription(ctx, &pubsub.SubscriptionConfig{
			Project: cfg.ProjectID,
			Topic:   topicName,
			Name:    subName,
		})
	...
	t.Run("Get or create subscription with nonexisting topic", func(t *testing.T) {
		...
		_, err := client.GetOrCreateSubscription(ctx, &pubsub.SubscriptionConfig{
			Project: cfg.ProjectID,
			Topic:   topicNoExist,
			Name:    subName,
		})
