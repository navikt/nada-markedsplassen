...
func NewArtifactRegistryCredentialService(enabled bool, repositoryIDs []string, totalTimeout time.Duration, api service.ArtifactKeeperAPI) service.ArtifactRegistryCredentialService {
	return &artifactRegistryCredentialService{
		enabled: enabled, repositoryIDs: repositoryIDs, api: api, totalTimeout: totalTimeout,
	}
}
...
func (s *artifactRegistryCredentialService) Prepare(ctx context.Context, workstationID string) (*service.ArtifactRegistryCredential, error) {
	...
	created, err := s.api.CreateToken(ctx, service.ArtifactKeeperCreateTokenRequest{
		Name: name, ExpiresInDays: 1, Scopes: []string{"read:artifacts"}, RepositoryIDs: s.repositoryIDs,
	})
	if err != nil {
