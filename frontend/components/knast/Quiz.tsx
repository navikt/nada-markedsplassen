...
const QUIZ_COOKIE_NAME = "sikkerhetsCrateOpened";

export const setQuizReadCookie = () => {
  const d = new Date();
  d.setMonth(d.getMonth() + 3);
  document.cookie = `${QUIZ_COOKIE_NAME}=${true}; expires=${d.toUTCString()}; path=/`;
}

export const getQuizReadCookie = () => {
  const value = `; ${document.cookie}`;
  const parts = value.split(`; ${QUIZ_COOKIE_NAME}=`);
...
const Quiz = ({ show, onClose }: QuizProps) => {
  ...
  return (
    <Modal width={"100[rem]"} open={show} aria-label="Sikkerhetsquiz" onClose={() => { }}>
      <Modal.Body>
