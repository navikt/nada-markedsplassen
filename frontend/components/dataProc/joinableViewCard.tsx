    ...
    return (
        <div>
            ...
            {data && <>
                ...
                {data?.pseudoDatasources.map((bqv:any, index: number) => <Box key={index} padding="space-4" className="w-[55rem]">
                    ...
                    bqv.accessible? <div className="flex flex-row items-center bg-ax-neutral-300">{bqv.bigqueryUrl}<CopyButton copyText={bqv.bigqueryUrl}></CopyButton></div>
                        : <Tooltip content="Har ikke tilgang til datasettet"><div className="flex flex-row items-center text-ax-neutral-300" >{bqv.bigqueryUrl}</div></Tooltip>}
                </Box>)}
                {data?.expires &&
